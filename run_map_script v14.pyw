...
# AED 13/07/22 - Base cost calculation on hourlyrate
...
# Would be nice to ensure has no trailing "s"
        ...
        cost = 0
#Base cost on hourly rate - AED 13/07/22
        hourlyrate = 80
...
# - AED 13/07/22
        ...
        if self.SLA.get() <> 1:
            if costCalculated:
# - AED 13/07/22
                emFile.write("\nOur invoice for " + unichr(163) + str(int(cost)) + " + VAT will follow in due course.\n\n")
            else:
...
version = 14
root.title("ERIC Mapping Script V"+str(version))
#AED 27/7/20
